...
def get_vision_backbone_and_transform(
    ...
    if vision_backbone_id == VISION_BACKBONE_ID:
    ...
    elif vision_backbone_id == LEVJEPA_VISION_BACKBONE_ID:
        backbone = LeVJEPAViTBackbone(
            vision_backbone_id,
            image_resize_strategy,
            checkpoint_path=checkpoint_path,
        )
        ...
        raise ValueError(
            f"Unsupported vision backbone `{vision_backbone_id}`. Expected one of "
            f"`{VISION_BACKBONE_ID}` or `{LEVJEPA_VISION_BACKBONE_ID}`."
        )
